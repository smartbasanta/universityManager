...
export const SearchBar = ({ 
  ...
  const getInputContainerClasses = () => {
    ...
    const focusClasses = isFocused ? 'ring-2 ring-green-500 ring-opacity-50' : '';
  ...
  const getIconContainerClasses = () => {
    ...
    const focusClasses = isFocused ? 'bg-[#e6f7ff]' : '';
    return `${baseClasses} ${paddingClass} ${focusClasses}`;
  };

  const getInputClasses = () => {
    const baseClasses = "form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl text-[#121416] focus:outline-0 focus:ring-0 border-none bg-[#f1f2f4] focus:border-none h-full placeholder:text-[#6a7581] rounded-l-none border-l-0 font-normal leading-normal transition-colors";
      ...
      {isMobile && showSuggestions && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
          {searchQuery.length === 0 && recentSearches.length > 0 ? (
            ...
            /* Search suggestions for opportunities */
            <div className="p-2">
              <div className="text-xs text-gray-500 px-2 mb-2">
                Suggested searches:
              </div>
              {['Competition', 'Research Program', 'Hackathon', 'Bootcamp', 'Startup'].filter(suggestion => 
      ...
      {isMobile && isFocused && (
        <div className="absolute top-full left-0 right-0 mt-1 px-2">
          <div className="text-xs text-gray-500">
            {searchQuery ? `Searching for "${searchQuery}"` : 'Start typing to search opportunities'}
          </div>
        </div>
      )}
    </div>
