...
export const FilterDropdowns = ({ 
  ...
  const SearchableDropdown = ({ id, label, options }: { id: string; label: string; options: string[] }) => {
    ...
    // Apply the same safety check we fixed for ambassadors to prevent toLowerCase errors
    const filteredOptions = options
      .filter(option => option != null && typeof option === 'string') // Filter out invalid values
      .filter(option => 
  ...
  return (
    <div className="flex gap-3 p-3 flex-wrap pr-4">
      {/* Higher Education Level commented out as requested */}
      {/* <DropdownButton
        id="educationLevel"
      ...
      {/* Major commented out as requested */}
      {/* <SearchableDropdown
        id="major"
        label="Major"
        options={majors}
