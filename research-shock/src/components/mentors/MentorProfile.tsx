...
import type { MentorDisplayData } from '@/types/mentors/mentor';
import type { SlotResponse } from '@/hooks/api/mentors/mentors.api';

interface MentorProfileProps extends MentorDisplayData {
  // Remove availableSlots from props since we'll fetch them
}
...
export const MentorProfile = ({
  ...
  // Fetch mentor's available slots using the hook
  const { 
    data: availableSlots = [], 
  ...
  const handleBookMeeting = () => {
    ...
    // Use the existing slot ID created by the mentor
    ...
    router.push(`/mentors/${id}/apply?${searchParams.toString()}`);
  };
  ...
  return (
    <div className="px-4 sm:px-8 md:px-20 lg:px-40 flex justify-center py-5">
      <div className="layout-content-container w-full max-w-[960px] flex flex-col">
        <Breadcrumb basePath="/mentors" baseLabel="Mentors" currentName={name} />
        ...
        <div className="flex p-4 gap-4 flex-col sm:flex-row items-center sm:items-start">
          ...
          <div className="text-center sm:text-left space-y-1">
            ...
            {educationLevel && <p className="text-[#5c738a]">{educationLevel}</p>}
            ...
            <p className="text-[#5c738a]">{position} at {company}</p>
            <p className="text-[#5c738a]">Alumni of {university}</p>
          </div>
        </div>
          ...
          <div className="px-4 pb-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
              <p className="text-yellow-800">
                This mentor hasn't created any available time slots yet.
              </p>
            </div>
          </div>
        )}
