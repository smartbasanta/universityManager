...
export const UndergraduateCard = ({ admissions, tuition, departments }: UndergraduateCardProps) => {
  const undergraduateAdmissions = admissions?.filter(a => a.level === 'undergraduate');
  const undergraduateTuition = tuition?.filter(t => t.level === 'undergraduate');

  return (
    <Card className="shadow-lg rounded-lg">
      <CardHeader>
        <CardTitle className="text-2xl font-bold">Undergraduate</CardTitle>
      </CardHeader>
      <CardContent className="space-y-8">
        {/* Admissions Section */}
        <section>
          <h3 className="text-xl font-bold text-gray-800 mb-4">Admissions</h3>
          {undergraduateAdmissions && undergraduateAdmissions.length > 0 ? (
            <div className="space-y-6">
              {undergraduateAdmissions.map((admission) => (
          ...
          ) : <p>No undergraduate admissions data available.</p>}
        </section>

        {/* Tuition Section */}
        <section>
          <h3 className="text-xl font-bold text-gray-800 mb-4">Tuition & Fees</h3>
          {undergraduateTuition && undergraduateTuition.length > 0 ? (
            <div className="space-y-6">
              {undergraduateTuition.map((fee) => (
          ...
          ) : <p>No undergraduate tuition data available.</p>}
        </section>
        ...
        <section>
          ...
          ) : <p>No undergraduate departments data available.</p>}
        </section>
      </CardContent>
    </Card>
  );
