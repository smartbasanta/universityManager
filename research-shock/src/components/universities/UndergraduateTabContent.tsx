...
interface UndergraduateTabContentProps {
  universityId: string;
}
...
export function UndergraduateTabContent({ universityId }: UndergraduateTabContentProps) {
  ...
  if (error) {
    return (
      <div className="text-center py-20 text-red-600">
        <p className="text-lg font-semibold">Failed to load undergraduate information</p>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {rankings.map((ranking) => (
                <div key={ranking.id} className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100">
                  ...
                  <p className="text-3xl font-bold text-blue-600 mb-2">#{ranking.rank}</p>
          ...
          >
            ...
            <div className="space-y-6">
              {departments.map((dept) => (
                <div key={dept.id} className="bg-green-50 rounded-xl p-6 border border-green-100">
                  ...
                  {dept.programs && dept.programs.length > 0 && (
                    <div className="mt-4">
                      <p className="text-sm font-semibold text-gray-700 mb-3">Programs Offered:</p>
                      <div className="flex flex-wrap gap-2">
                        {dept.programs.map((program) => (
                          <span 
                            key={program.id}
                            className="px-3 py-1 bg-white text-gray-700 rounded-full text-sm border border-green-200"
                          >
                            {program.name}
                          </span>
                        ))}
              ...
              <div key={admission.id} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-purple-50 rounded-xl p-6 text-center border border-purple-100">
                    ...
                    <p className="text-4xl font-bold text-purple-600">{parseFloat(admission.acceptance_rate).toFixed(0)}%</p>
                  </div>
                  ...
                  {admission.application_fee && (
                    <div className="bg-purple-50 rounded-xl p-6 text-center border border-purple-100">
                      ...
                      <p className="text-3xl font-bold text-purple-600">${admission.application_fee}</p>
                    </div>
                  )}
                </div>
          ...
          >
            ...
            <div className="space-y-6">
              {tuition_fees.map((tuition) => (
                <div key={tuition.id} className="bg-orange-50 rounded-xl p-6 border border-orange-100">
