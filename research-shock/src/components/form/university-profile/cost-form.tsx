...
import { useAddUniversityTuitionCost, useGetUniversityTuitionCost } from "@/hooks/api/university/university.query";

export default function CostForm() {
  const [tuition, setTuition] = useState([{ type: "In State", cost: 0 }]);
  const [averageAid, setAverageAid] = useState(""); // Initialize with empty string
  const [studentsReceivingAid, setStudentsReceivingAid] = useState(""); // Initialize with empty string
  const [housingCost, setHousingCost] = useState(""); // Initialize with empty string
  const [mealPlanCost, setMealPlanCost] = useState(""); // Initialize with empty string
  const [bookSuppliesCost, setBookSuppliesCost] = useState(""); // Initialize with empty string
  
  // New state for read-only mode and edit functionality
  const [isReadOnly, setIsReadOnly] = useState(false);
  const [isFormSubmitted, setIsFormSubmitted] = useState(false);
  
  const { mutate: addUniversityTuitionCost, isPending } = useAddUniversityTuitionCost()
  const { data, isLoading, error } = useGetUniversityTuitionCost();

  // Populate form fields with existing data if available
  useEffect(() => {
    if (data) {
      setAverageAid(data?.avg_financial_aid?.toString() || "");
      setStudentsReceivingAid(data?.student_receiving_aid?.toString() || "");
      setHousingCost(data?.housing_cost?.toString() || "");
      setMealPlanCost(data?.meal_plan_cost?.toString() || "");
      setBookSuppliesCost(data?.books_supplies_cost?.toString() || "");
      
      if (data?.tuition_type && data.tuition_type.length > 0) {
        setTuition(data.tuition_type.map((item: any) => ({
          type: item.tuition_type || "In State",
          cost: Number(item.annual_cost) || 0,
        })));
      }
  ...
  const handleSaveDraft = () => {
    const payload = {
      ...
      isDraft: true,
      tuition_type: tuition.map(item => ({
        tuition_type: item.type,
        annual_cost: item.cost.toString(),
      })),
    };
    addUniversityTuitionCost(payload);
    setIsFormSubmitted(true);
    setIsReadOnly(true);
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={item.cost || ""} // Add fallback to empty string
                    onChange={(e) => {
                      const newTuition = [...tuition];
                      newTuition[index].cost = Number(e.target.value) || 0;
                      setTuition(newTuition);
                    }}
                    readOnly={isReadOnly}
                    disabled={isReadOnly}
