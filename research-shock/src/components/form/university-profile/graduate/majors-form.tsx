...
import { useAddUniversityGraduateMajor, useGetUniversityGraduateMajor } from "@/hooks/api/university/university.query";
...
export default function MajorForm() {
  ...
  const { mutate: addUniversityGraduateMajor, isPending: isSaving } = useAddUniversityGraduateMajor();
  const { data: majorResponse, isLoading: isLoadingMajor, error: majorError } = useGetUniversityGraduateMajor();

  // Populate form fields with existing data if available
  useEffect(() => {
    if (majorResponse && Array.isArray(majorResponse) && majorResponse.length > 0) {
      setMajors(majorResponse.map((major: any) => ({
        name: major.name || "",
        rank: major.rank || "",
        link: major.link || ""
      })));
  ...
  const createPayload = (isDraft: boolean) => {
    // Filter out empty majors
    const filteredMajors = majors.filter(
      (major) => major.name.trim() !== "" || major.rank.trim() !== "" || major.link.trim() !== ""
    );
  ...
  const handleSaveDraft = () => {
    ...
    addUniversityGraduateMajor(payload);
    setIsFormSubmitted(true);
    setIsReadOnly(true);
  };
  ...
  if (majorError) {
    return <div>Error: {majorError.message}</div>;
  }
