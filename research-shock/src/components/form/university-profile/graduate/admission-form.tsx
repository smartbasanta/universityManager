...
import { useAddUniversityGraduateAdmission, useGetUniversityGraduateAdmission } from "@/hooks/api/university/university.query";

export default function AdmissionForm() {
  const { mutate: addUniversityGraduateAdmission, isPending: isSaving } = useAddUniversityGraduateAdmission();
  const { data: admissionResponse, isLoading: isLoadingAdmission, error: admissionError } = useGetUniversityGraduateAdmission();
  ...
  const handleSaveDraft = () => {
    ...
    addUniversityGraduateAdmission(payload);
    setIsFormSubmitted(true);
    setIsReadOnly(true);
  };
            ...
            <div key={index} className="p-4 pb-4 space-y-4 bg-gray-200 rounded-lg mb-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    placeholder="e.g., 260-340, 205-805"
                    className="flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-[#111418] focus-visible:ring-0 focus-visible:ring-offset-0 border-none bg-[#f0f2f5] h-14 placeholder:text-[#60748a] p-4 text-base font-normal leading-normal"
