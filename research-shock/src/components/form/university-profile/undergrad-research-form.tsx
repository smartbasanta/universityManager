"use client";

import { useState, useEffect } from "react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Trash2, Plus, Edit } from "lucide-react";
import { useAddUniversityResearchOppertunity, useGetUniversityResearchOppertunity } from "@/hooks/api/university/university.query";

export default function UndergradResearchForm() {
  const [research, setResearch] = useState([
    {
      researchCenter: "",
      facultySupervisor: "",
      projectTitle: "",
      url: "",
    },
  ]);
  
  // New state for read-only mode and edit functionality
  const [isReadOnly, setIsReadOnly] = useState(false);
  const [isFormSubmitted, setIsFormSubmitted] = useState(false);
  
  const { mutate: addUniversityResearchOppertunity, isPending } = useAddUniversityResearchOppertunity()
  const { data, isLoading, error } = useGetUniversityResearchOppertunity()

  // Populate form fields with existing data if available
  useEffect(() => {
    if (data) {
      const formattedData = data.map((item: any) => ({
        researchCenter: item.research_center,
        facultySupervisor: item.faculty_supervisor,
        projectTitle: item.project_title,
        url: item.url,
      }));
      setResearch(formattedData);
      
      // Set form as submitted and read-only since data exists
      setIsFormSubmitted(true);
      setIsReadOnly(true);
    }
  }, [data]);

  const handleAddResearchOpportunity = () => {
    if (isReadOnly) return;
    setResearch([...research, { researchCenter: "", facultySupervisor: "", projectTitle: "", url: "" }]);
  };

  const handleRemoveResearchOpportunity = (index: any) => {
    if (isReadOnly) return;
    setResearch(research.filter((_, i) => i !== index));
  };

  const handleSaveDraft = () => {
    const payload = research.map(item => ({
      research_center: item.researchCenter,
      faculty_supervisor: item.facultySupervisor,
      project_title: item.projectTitle,
      url: item.url,
      isDraft: true,
    }));
    addUniversityResearchOppertunity(payload);
    setIsFormSubmitted(true);
    setIsReadOnly(true);
  };

  const handleSubmit = () => {
    const payload = research.map(item => ({
      research_center: item.researchCenter,
      faculty_supervisor: item.facultySupervisor,
      project_title: item.projectTitle,
      url: item.url,
      isDraft: false,
    }));
    addUniversityResearchOppertunity(payload);
    setIsFormSubmitted(true);
    setIsReadOnly(true);
  };

  const handleEditInfo = () => {
    setIsReadOnly(false);
  };

  if (isLoading) {
    return <div>Loading...</div>
  }
  if (error) {
    return <div>Error: {error.message}</div>
  }

  return (
    <div className="flex flex-col max-w-[960px] flex-1 mt-5">
      <form className="space-y-6">
        {/* Undergraduate Research */}
        <div className="space-y-4 px-4">
          {research.map((item, index) => (
            <div key={index} className="p-4 pb-4 space-y-4 bg-gray-200 rounded-lg mb-2">
              <div className="flex justify-between items-center mb-3">
                <h4 className="font-medium text-gray-600">Research Opportunity #{index + 1}</h4>
                {index > 0 && !isReadOnly && (
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    onClick={() => handleRemoveResearchOpportunity(index)}
                  >
                    <Trash2 className="h-4 w-4 text-destructive" />
                  </Button>
                )}
              </div>

              <div>
                <label className="text-gray text-base font-medium leading-normal">Research Center</label>
                <Input
                  placeholder="e.g., Bioinformatics Lab"
                  className="flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-[#111418] focus-visible:ring-0 focus-visible:ring-offset-0 border-none bg-[#f0f2f5] h-14 placeholder:text-[#60748a] p-4 text-base font-normal leading-normal"
                  value={item.researchCenter}
                  onChange={(e) => {
                    const newResearch = [...research];
                    newResearch[index].researchCenter = e.target.value;
                    setResearch(newResearch);
                  }}
                  readOnly={isReadOnly}
                  disabled={isReadOnly}
                />
              </div>

              <div>
                <label className="text-gray text-base font-medium leading-normal">Faculty Supervisor</label>
                <Input
                  placeholder="e.g., Dr. Jane Doe"
                  className="flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-[#111418] focus-visible:ring-0 focus-visible:ring-offset-0 border-none bg-[#f0f2f5] h-14 placeholder:text-[#60748a] p-4 text-base font-normal leading-normal"
                  value={item.facultySupervisor}
                  onChange={(e) => {
                    const newResearch = [...research];
                    newResearch[index].facultySupervisor = e.target.value;
                    setResearch(newResearch);
                  }}
                  readOnly={isReadOnly}
                  disabled={isReadOnly}
                />
              </div>

              <div>
                <label className="text-gray text-base font-medium leading-normal">Project Title</label>
                <Input
                  placeholder="e.g., AI for Healthcare"
                  className="flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-[#111418] focus-visible:ring-0 focus-visible:ring-offset-0 border-none bg-[#f0f2f5] h-14 placeholder:text-[#60748a] p-4 text-base font-normal leading-normal"
                  value={item.projectTitle}
                  onChange={(e) => {
                    const newResearch = [...research];
                    newResearch[index].projectTitle = e.target.value;
                    setResearch(newResearch);
                  }}
                  readOnly={isReadOnly}
                  disabled={isReadOnly}
                />
              </div>

              <div>
                <label className="text-gray text-base font-medium leading-normal">URL</label>
                <Input
                  placeholder="https://research.university.edu/project"
                  className="flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-[#111418] focus-visible:ring-0 focus-visible:ring-offset-0 border-none bg-[#f0f2f5] h-14 placeholder:text-[#60748a] p-4 text-base font-normal leading-normal"
                  value={item.url}
                  onChange={(e) => {
                    const newResearch = [...research];
                    newResearch[index].url = e.target.value;
                    setResearch(newResearch);
                  }}
                  readOnly={isReadOnly}
                  disabled={isReadOnly}
                />
              </div>
            </div>
          ))}
          {!isReadOnly && (
            <Button
              type="button"
              variant="outline"
              onClick={handleAddResearchOpportunity}
              className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-[#f0f2f5] text-[#111418] text-sm leading-normal tracking-[0.015em] hover:bg-gray-300 w-full md:w-max"
            >
              <Plus className="w-4 h-4 mr-2" />
              Add Research Opportunity
            </Button>
          )}
        </div>

        {/* Form Actions */}
        <div className="flex px-4 py-3 justify-end gap-4">
          {isFormSubmitted && isReadOnly && (
            <Button
              type="button"
              variant="outline"
              onClick={handleEditInfo}
              className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-[#6c757d] text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[#6c757d]/90"
            >
              <Edit className="w-4 h-4 mr-2" />
              Edit Info
            </Button>
          )}
          {!isReadOnly && (
            <>
              <Button
                type="button"
                variant="outline"
                onClick={handleSaveDraft}
                disabled={isPending}
                className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-[#0c77f2] text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[#0c77f2]/90"
              >
                Save as Draft
              </Button>
              <Button
                type="button"
                onClick={handleSubmit}
                disabled={isPending}
                className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-[#198754] text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[#198754]/90"
              >
                Submit
              </Button>
            </>
          )}
        </div>
      </form>
    </div>
  );
}
