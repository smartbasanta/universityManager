...
import { useGetAmbassadorProfile, useUpdateAmbassadorProfile } from '@/hooks/api/ambassadors/ambassadors.query';
import { useCreateAmbassadorSlot, useGetAllAmbassadorSlots, useDeleteAmbassadorSlot } from '@/hooks/api/ambassadors/ambassadors.query';
...
export const AmbassadorDashboard = () => {
  ...
  const [slotDuration, setSlotDuration] = useState('15'); // Default to 15 minutes

  // Profile form state
  const [name, setName] = useState('');
  const [email, setEmail] = useState(''); // Keep empty as API doesn't have it
  ...
  // API hooks
  const { data: profileData, isLoading, error } = useGetAmbassadorProfile();
  const { mutate: updateProfile, isPending: isUpdating } = useUpdateAmbassadorProfile();
  
  // Updated: Using ambassador-specific slot hooks
  const { data: allSlots = [], isLoading: slotsLoading, refetch: refetchSlots } = useGetAllAmbassadorSlots();
  const createSlotMutation = useCreateAmbassadorSlot();
  const deleteSlotMutation = useDeleteAmbassadorSlot();
  

  // Filter slots to show only current ambassador's slots
  const ambassadorSlots = allSlots.filter(slot => {
    // If the API doesn't return ambassadorId, you might need to filter differently
  ...
  useEffect(() => {
    if (profileData && !isDataLoaded) {
      console.log("Loading profile data from API:", profileData);
      ...
      setAccess(profileData.profileStatus === 'public' ? 'public' : 'private');
      ...
      <div className="flex flex-col grow h-full">
        <div className="flex flex-1 justify-center px-6 py-10">
          <div className="flex flex-col w-full max-w-3xl gap-10">
            ...
            {/* Section 1: Ambassador Profile */}
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold text-gray-800">Student Ambassador Profile</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input 
                    type="email" 
                    className="w-full border border-gray-300 p-2 rounded" 
                    placeholder="Email managed by admin"
                    value={email}
                    readOnly={isReadOnly}
                    disabled={isReadOnly}
                  />
              ...
              <div>
                ...
                <input 
                  type="text" 
                  className={`w-full border border-gray-300 p-2 rounded ${isReadOnly ? 'bg-gray-100' : ''}`}
                  placeholder="e.g., Frederick Douglass High School, 2018"
                  value={education}
...
<div className="space-y-6 border-t border-gray-200 pt-8">
  ...
  {/* Display Added Slots - Updated to show booking status */}
  <div className="mt-4">
    {slotsLoading ? (
      <div className="text-center py-4 text-gray-500">Loading slots...</div>
    ) : ambassadorSlots.length === 0 ? (
  ...
  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
    <p><strong>Note:</strong> These time slots will be visible to students on your ambassador profile page. Students can book available slots for ambassador sessions.</p>
    {!isReadOnly && (
