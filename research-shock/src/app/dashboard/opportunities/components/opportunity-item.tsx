...
import { useDeleteOpportunity, useUpdateOpportunityStatus } from "@/hooks/api/opportunity-hub/opportunity.query";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export const OpportunityItem = ({
  ...
  const { mutate: updateStatus, isPending: isUpdatingStatus } = useUpdateOpportunityStatus();
  ...
  const getEditUrl = () => {
    if (!id) return "#";
    return `/dashboard/opportunities/edit/${id}?status=${status || 'draft'}`;
  };

  // Get applications URL
  console.log('Debug info:', { status, id, shouldShow: status === 'Live' && id });
  const getApplicationsUrl = () => {
    if (!id) return "#";
    return `/dashboard/opportunities/applications/${id}`;
  };
  ...
  return (
    <div className="flex items-center gap-4 bg-white px-4 min-h-[72px] py-2 justify-between">
      ...
      <div className="shrink-0 flex gap-2">
        {/* View Applications Button - Show only for Live opportunities */}
        {status === 'Live' && id && (
          <ProtectComponent requiredRoles={[roleType.UNIVERSITY, roleType.UNIVERSITY_STAFF, roleType.INSTITUTION, roleType.DEPARTMENT_STAFF, roleType.INSTITUTION_STAFF]} requiredPermission={[permissionType.VIEW_OPPERTURUNITY]}>
        ...
        {/* Edit Button */}
        {id ? (
          <ProtectComponent requiredRoles={[roleType.UNIVERSITY, roleType.UNIVERSITY_STAFF, roleType.INSTITUTION, roleType.DEPARTMENT_STAFF, roleType.INSTITUTION_STAFF]} requiredPermission={[permissionType.EDIT_OPPERTURUNITY]}>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Are you sure you want to move "{title}" to Live? This will publish the opportunity.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Are you sure you want to move "{title}" to Archive? This will unpublish the opportunity.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>Cancel</AlertDialogCancel>
        ...
        <ProtectComponent requiredRoles={[roleType.UNIVERSITY, roleType.UNIVERSITY_STAFF, roleType.INSTITUTION, roleType.DEPARTMENT_STAFF, roleType.INSTITUTION_STAFF]} requiredPermission={[permissionType.DELETE_OPPERTURUNITY]}>
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button
                variant="destructive"
